...
public class EsQueryClientUtils {

	private static String esUrl = "http://127.0.0.1:9200";

	private static RestClient restClient = RestClient.builder(HttpHost.create(esUrl)).build();

	/** logger */
	private static final Logger log = LoggerFactory.getLogger(EsQueryClientUtils.class);
	...
	public static <T> List<T> query(String sql, Map<String, Object> params, Class<T> className) {
		...
		JSONObject response = JSONUtil.parseObj(send(sql, restClient));
...
//        List<List> rows = JSONObject.parseArray(response.getString("rows"), List.class);
	 ...
	 * @return
	 */
	public static String send(String sql, HttpHost httpHost) {
		return send(sql, RestClient.builder(httpHost).build());
	 ...
	 * @return
	 */
	public static String send(String sql, RestClient restClient) {
		try {
