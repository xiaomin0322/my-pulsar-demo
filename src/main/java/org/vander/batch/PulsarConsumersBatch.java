...
public class PulsarConsumersBatch {
	...
	public static void main(String[] args) throws Exception {
		...
		consumer = client.newConsumer().topic("my-topic").ackTimeout(30, TimeUnit.SECONDS)
				.batchReceivePolicy(BatchReceivePolicy.builder().maxNumMessages(100).maxNumBytes(1024 * 1024).build())
				.subscriptionName("my-subscription").subscriptionType(SubscriptionType.Shared)
				.deadLetterPolicy(
						DeadLetterPolicy.builder().maxRedeliverCount(10).deadLetterTopic("dl-topic-name").build())
				.subscribe();
